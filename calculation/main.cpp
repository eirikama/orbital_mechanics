  ...
  for (const auto &body : data["celestial_bodies"]) {
    ...
    if (name != "Sun") {
      ...
      std::vector<double> position =
          body["position"].get<std::vector<double>>();
      ...
      SolarSystem.AddBody(position[0], position[1], position[2], velocity[0],
                          velocity[1], velocity[2], mass, name);
    }
  }
